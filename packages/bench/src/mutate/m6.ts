...
export function mutateM6(snapshot: unknown, seed: number): MutationResult {
  ...
  if (!target) throw new Error('mutateM6: no element with an id attribute found in this snapshot');

  const t = target as Tree;
  const attrs = t[1] as Record<string, string>;
  const originalSelector = `#${attrs.id}`;
  markTarget(t);

  // Build a near-identical decoy: same tag, no id collision, a text variant close
  // to the original (so accessible-name similarity stays high without being exact).
  const textChild = t.slice(2).find((c) => typeof c === 'string') as string | undefined;
  const decoyText = textChild ? `${textChild} draft` : undefined;
  const decoy: Tree = [
    t[0] as string,
    { id: `${attrs.id}-decoy-${seed}` },
    ...(decoyText ? [decoyText] : []),
  ];
